...
for item, pagina in sumario:
    p = doc.add_paragraph()
    # Adicionar tabulação para subitens
...
for print_num, local, descricao in lista_prints:
    p = doc.add_paragraph()
    run = p.add_run(f'{print_num}\t{local}\t{descricao}')
    run.font.size = Pt(10)
...
# Salvar
doc.save('/root/.openclaw/workspace/PROJETOS/FACULDADE/Sistemas-Distribuidos/RELATORIO-SISTEMAS-DISTRIBUIDOS-V2.docx')
print('✅ DOCX V2 criado com sucesso! (sem tabelas, com tabulações)')
