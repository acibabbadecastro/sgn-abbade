#!/bin/bash
# generate-report.sh - Gera documentação final de acessos
...
echo -e "${BLUE}📋 GENERATE REPORT - Documentação de Acessos${NC}"
echo "=============================================="
echo ""

WORK_DIR="/root/.openclaw/workspace/skills/auto-config-detector"
REPORT_FILE="$WORK_DIR/references/acessos-auto-detect.md"
VAULT_DIR="$HOME/.openclaw/secure"

mkdir -p "$WORK_DIR/references"

echo -e "${YELLOW}[1/3]${NC} Gerando documentação..."
...
echo -e "${YELLOW}[2/3]${NC} Documentando infraestrutura..."
...
echo -e "${YELLOW}[3/3]${NC} Documentando desenvolvimento e projetos..."
...
echo "   Linhas: $(wc -l < $REPORT_FILE)"
echo ""
echo -e "${BLUE}📋 PRÓXIMOS PASSOS:${NC}"
echo "   1. Revisar relatório: cat $REPORT_FILE"
echo "   2. Sync para DATASVR (automático)"
echo "   3. Nova IA lê e usa sem perguntar logins!"
echo ""
echo -e "${YELLOW}💡 DICA:${NC} Execute este script após cada /new ou nova config"
